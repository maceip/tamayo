...
across the stack. `tamayo` owns the Go, cgo-free cryptographic primitives and
the token-profile building blocks. Product runtimes should import these pieces;
they should not invent parallel token formats.

## Repository Boundaries

| repo | role | direction |
| --- | --- | --- |
| `tamayo` | Go/TamaGo crypto and token-profile primitives | Keep and make authoritative. |
...
| 4 | Reference email token | Verified email address only. | Interop and regression testing against the public email-verification-token shape. | JWT-style expiry plus presentation nonce/key binding. | None by design. This row is intentionally not coupled to TEE measurement, policy, or PQ. | No: classical JOSE path. | Implemented most fully in `eat-pass bridge`; should be ported or kept as a reference profile in Go without coupling it to private-token policy. |
...
- Row 4 exists to keep an unmodified reference email-token implementation
  available for testing and interop. It should stay separate from the
...
3. Port the reference email-token issuer/verifier from `eat-pass bridge` into a
   Go profile, separate from private-token policy.
4. Add explicit tests for rows 1-3 proving that accepted runtime measurements
   can authorize minting or issuance.
5. Remove or thin the duplicate token-service implementation in
   `confidential-agent` after the Go profiles are available from `tamayo`.
6. Make `confidential-agent` a consumer: bootable runtime, agent workflow, and
   policy wiring only.
